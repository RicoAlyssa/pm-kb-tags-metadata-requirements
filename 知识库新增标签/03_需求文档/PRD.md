...
状态：评审中草稿（瀚海与当前星流平台 API Explorer 均未发现标签接口；缺计费归属模型、控制台截图和 Pencil 原型确认，不能标“已批准”）
...
- 我方底层：当前可见瀚海 OPEN-API 目录未发现标签接口，状态 `TO_CONFIRM`。
- 我方当前平台：金山云 API Explorer / 星流平台知识库接口未发现知识库级标签、文档级标签、标签绑定/解绑或标签过滤字段。
- 正式竞品：火山云公开文档支持文档标签、权限标签过滤和 `doc_filter`；阿里云百炼公开文档支持文档标签、API `tags` 参数、检索服务中的标签过滤。
- 参考实现：Dify 标签 API 只作为接口形态参考，不进入正式竞品结论。
...
| 当前平台接入 | 星流平台 OpenAPI/控制台暴露标签能力 | TO_CONFIRM | 明确接口命名、鉴权、错误码、审计和发布版本 |
...
| TAG-Q-006 | TO_CONFIRM | 当前星流平台是否接入标签能力以及上线版本 | 平台后端 | 阻塞接口联调 |
