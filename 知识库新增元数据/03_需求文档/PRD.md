...
| P1 | 内置字段开关、使用量、引用关系、删除影响详情 | 可借鉴 Dify built-in/custom |
| P1 | 导入/导出、批量清空、模板化元数据 | 依赖 P0 稳定 |
| P2 | 自动抽取、正则/LLM 生成元数据 | 可参考阿里云做后续能力 |
...
|---|---|---|---|---|---|
| `STATE-META-DEFINITION-LIST.png` | 知识库元数据定义列表 | 展示字段名、类型、使用文档数、内置/自定义、更新时间 | 搜索、创建、编辑、删除、查看引用 | 空态、无权限、加载失败 | 参考 META-EV-0004；我方待 Pencil |
| `STATE-META-CREATE.png` | 新增元数据字段弹窗/抽屉 | 字段名、字段类型、说明（如需要） | 选择类型、输入名称、保存/取消 | 重名、非法字符、类型缺失 | 参考 Dify；我方接口待确认 |
| `STATE-META-EDIT.png` | 编辑元数据字段 | 已有字段名回填；类型是否锁定 | 改名保存；查看影响范围 | 类型不可改、被检索配置引用、并发修改 | 参考 META-EV-0005 |
...
| `STATE-META-BATCH.png` | 文档列表批量元数据编辑器 | 多选文档后展示字段和值编辑区 | 添加已有字段/新字段、应用范围、保存 | 部分失败、覆盖/部分更新、清空值 | 参考 Dify 批量编辑；内部接口待确认 |
| `STATE-META-RETRIEVAL.png` | 召回测试/应用配置 metadata filter | 展示字段、运算符、值、组合关系 | 添加/删除条件，测试召回，查看过滤结果 | 字段被删、值类型不匹配、无命中文档 | 参考 META-EV-0006/0009/0010 |
